...
安卓端也可以选择下载[船新版本](../../releases/latest)；也可以选择[我的服务器下载](https://南林图书馆.中国/download/AutoLib-0.2.3.apk) (注意这里之后只会停留在0.2.3版本，最新请在release中下载或下载后点击检查更新)
...
- **桌面小组件**三种规格，不打开 App 也能看今天坐哪：
  ...
  - 取消预约在桌面上二次确认后直接生效；午休 / 调整明日会跳进 App 对应界面
- **抢座结果通知**：每天 07:05 自动查询前一晚的抢座结果并推送，默认开启
- **闭馆公告提醒**：人工确认学校公告后，App 每天 20:30（北京时间）后台检查，并在前台首次打开时弹窗
- **按图选座**：12 张楼层平面图，拖动缩放，点哪儿就选中离它最近的那个座位；图上标出自己的优先级顺序，以及「除你以外还有几个人也把它放进了优先级」，避开一堆人盯着的位置
- **邮箱通知分两档**：「仅异常」只在抢座、午休失败时来信，「全部」每天成功也发，含座位号和时段
- **学习记录热力图**：按学期统计每天的自习时长，颜色深浅一目了然
- **主页一屏看全**：今日、明日、快捷调整明日时段、本周配置预览
- 是否到馆由服务器自动判断（刷卡进馆即识别），不再需要手动点「我已到馆」；已入座时「取消」会变成「离馆」
- 跟随系统 / 亮色 / 暗色三档主题
