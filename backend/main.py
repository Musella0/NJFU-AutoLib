...
@login_required
def reserve_now(pid):
    """Trigger an immediate reservation for a specific library account"""
    import threading
    uid = session["web_uid"]
    client, db = get_db()
    cfg = db.user_config_info.find_one({"pid": pid, "web_uid": uid})
    ...
    if not cfg.get("seat_list"):
        return jsonify({"error": "请先配置座位优先级"}), 400

    def run():
        from scheduled_task import reservation
        reservation(cfg)

    threading.Thread(target=run, daemon=True).start()
    return jsonify({"message": f"已为学号 {pid} 触发立即抢座，结果稍后可在通知栏查看"}), 200
...
def list_announcements():
    """Public (logged-in) list of active announcements for users."""
    client, db = get_db()
    docs = list(
