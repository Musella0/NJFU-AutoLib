...
跑在哪
------
设计上跑在分机部署的 edge（境外那台，只有 Caddy，7:00 闲着）。它和抢座的
backend 不是同一台机器、不是同一条出口，采样再密也不会跟 7:00 的下单抢带宽。
...
用法（edge 上，挂工作区、连本机 mongo）::

    docker run -d --name autolib-rush-probe --network autolib_default \\
      --env-file .env -e DB_IP=mongo:27017 -e TZ=Asia/Shanghai \\
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    open_at = _parse_open(args.open, datetime.now().date())
    if args.date:
